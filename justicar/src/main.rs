...
fn main() {
    ...
    let (raw_quote,quote_collateral) =if let types::AttestationReport::SgxDcap{quote: raw_quote,collateral: c,} = att_report {
    ...
    let (report_data,tcb_hash,tcb_status,advisory_ids) = match dcap::verify(&raw_quote, &quote_collateral, now) {
        Ok(r) => (r.0,r.1,r.2,r.3),
        Err(e) => {
            panic!("fail to verify report :{:?}",e)
        }
    };
    println!("report data is :{:?}",String::from_utf8(report_data.to_vec()));
